    ...
    <>
      <section className="border-b border-line py-8 px-6 dark:border-night-line">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-3xl font-bold text-ink mb-2 dark:text-night-ink">
              ...
              <div ref={wrapperRef} className="relative max-w-sm">
                ...
                <div className="flex gap-2">
                  ...
                  {(zoek || geselecteerd) && (
                    <button
                      onClick={wisSelectie}
                      className="text-xs text-sub underline whitespace-nowrap dark:text-night-sub"
                    >
                      Wissen
                    </button>
                  )}
                ...
                {open && zoek.trim() !== '' && !geselecteerd && (
                  <ul className="absolute z-10 mt-1 w-full bg-white border border-line rounded-lg shadow-lg max-h-64 overflow-y-auto dark:bg-night-panel dark:border-night-line">
                    {suggesties.length === 0 ? (
                      <li className="px-3 py-2 text-sm text-sub dark:text-night-sub">
                      ...
                      suggesties.map((s) => (
                        <li key={s.id}>
                          <button
                            ...
                            className="w-full text-left px-3 py-2 text-sm text-ink hover:bg-aqua-light/40 dark:text-night-ink dark:hover:bg-white/[0.05]"
                          >
                            {s.naam}
                            {s.rang && (
                              <span className="text-sub dark:text-night-sub"> &middot; #{s.rang}</span>
                            )}
                          </button>
                        </li>
                      ))
  ...
  return (
    <div>
      <div className="flex flex-wrap items-baseline justify-between gap-2 mb-6">
        <div>
          <h2 className="text-xl font-bold text-ink dark:text-night-ink">{swimmer.naam}</h2>
          <p className="text-sm text-sub mt-0.5 dark:text-night-sub">
      ...
      <div className="flex items-center gap-4 text-xs text-sub mb-4 dark:text-night-sub">
        <span>
        ...
        <>
          ...
          <ul className="sm:hidden space-y-2">
            {rijen.map((r) => (
              <li
                key={r.key}
                className="border border-line rounded-lg p-3 flex justify-between items-center dark:border-night-line"
              >
                <span className="text-sm text-ink dark:text-night-ink">{r.label}</span>
                <span
                  className={`font-mono text-sm ${
                    r.tijd ? 'text-ink dark:text-night-ink' : 'text-sub/40 dark:text-night-sub/40'
                  }`}
                >
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm max-w-md">
              ...
              <tbody>
                {rijen.map((r) => (
                  <tr key={r.key} className="border-b border-line dark:border-night-line">
                    <td className="py-2 pr-3 text-sub dark:text-night-sub">{r.label}</td>
                    <td
      ...
      <p className="text-xs text-sub mt-6 dark:text-night-sub">
...
function Statkaart({ label, waarde }: { label: string; waarde: string }) {
  return (
    <div className="border border-line rounded-lg p-3 dark:border-night-line">
      <div className="text-xs text-sub dark:text-night-sub">{label}</div>
      <div className="font-mono text-lg text-ink mt-0.5 dark:text-night-ink">{waarde}</div>
    </div>
  );
}
